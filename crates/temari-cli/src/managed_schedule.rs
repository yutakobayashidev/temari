...
pub(crate) enum SchedulerPlatform {
    #[default]
    Auto,
    Systemd,
    Launchd,
...
impl SchedulerPlatform {
    pub(crate) fn resolve(self) -> Result<Self> {
        match self {
...
impl ScheduleSpec {
    ...
    pub(crate) fn interval_seconds(&self) -> u32 {
        self.interval_seconds
    }
}

#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
pub(crate) struct ScheduleDefinition {
...
pub(crate) struct ScheduleStatus {
    pub(crate) platform: SchedulerPlatform,
    pub(crate) installed: bool,
    pub(crate) enabled: bool,
    pub(crate) active: bool,
    pub(crate) definition_paths: Vec<PathBuf>,
}
...
pub(crate) fn uninstall_schedule(
    workspace_id: &str,
    platform: SchedulerPlatform,
) -> Result<ScheduleStatus> {
    validate_workspace_id(workspace_id)?;
