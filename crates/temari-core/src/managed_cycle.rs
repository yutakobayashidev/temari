...
use crate::{
    ...
    Proposal, ScanScope, build_plan, canonical_source_identity, managed::MANAGED_AREAS,
    scan_directory,
};

pub const MANUAL_LIBRARY_DIRECTORY: &str = MANAGED_AREAS[0];
pub const RECENTS_DIRECTORY: &str = MANAGED_AREAS[1];
pub const AI_LIBRARY_DIRECTORY: &str = MANAGED_AREAS[2];
pub const STAGE_TO_RECENTS_RULE_ID: &str = "managed-stage-to-recents";
...
impl ManagedReprocessArea {
    fn directory(self) -> &'static str {
        match self {
            Self::ManualLibrary => MANUAL_LIBRARY_DIRECTORY,
            Self::AiLibrary => AI_LIBRARY_DIRECTORY,
        }
    }
}

/// Bind an approved folder set to the managed AI Library without changing any
/// destination IDs or classification metadata.
pub fn ai_library_folder_set(approved: &FolderSet) -> Result<FolderSet, Error> {
    approved.validate()?;
    let mut managed = approved.clone();
    managed.scope = ScanScope::new(vec![RECENTS_DIRECTORY.into()])?;
    for folder in &mut managed.folders {
        if first_component(&folder.path).eq_ignore_ascii_case(AI_LIBRARY_DIRECTORY) {
            return Err(Error::InvalidArtifact(format!(
                "approved destination is already inside {AI_LIBRARY_DIRECTORY}: {:?}",
                folder.path
            )));
        }
        folder.path = format!("{AI_LIBRARY_DIRECTORY}/{}", folder.path);
    }
    managed.validate()?;
    Ok(managed)
}
...
pub fn root_file_candidates(source: &Path) -> Result<Vec<FileCandidate>, Error> {
    ...
    scan_directory(
        ...
        &ScanScope::default(),
        &[
            MANUAL_LIBRARY_DIRECTORY.into(),
            RECENTS_DIRECTORY.into(),
            AI_LIBRARY_DIRECTORY.into(),
        ],
    )
}

/// Return only regular, non-symlink files directly inside Recents. Candidate
/// paths remain relative to the managed root, not to Recents itself.
pub fn recents_file_candidates(source: &Path) -> Result<Vec<FileCandidate>, Error> {
    let (source, _) = canonical_source_identity(source)?;
    let recents = source.join(RECENTS_DIRECTORY);
    let (resolved_recents, recents_identity) = canonical_source_identity(&recents)?;
    if resolved_recents != recents {
        return Err(Error::InvalidArtifact(
            "managed Recents must resolve inside the managed source".into(),
        ));
    }

    let mut candidates = scan_directory(&resolved_recents, &ScanScope::default(), &[])?;
    let (resolved_after_scan, identity_after_scan) = canonical_source_identity(&recents)?;
    if resolved_after_scan != resolved_recents || identity_after_scan != recents_identity {
        return Err(Error::InvalidArtifact(
            "managed Recents changed while it was being scanned".into(),
        ));
    }
    for candidate in &mut candidates {
        candidate.source_path = format!("{RECENTS_DIRECTORY}/{}", candidate.source_path);
    }
    Ok(candidates)
}

/// Enumerate explicitly selected files below Manual Library or AI Library for
/// staging back to Recents. Manual Library deliberately
...
pub fn reprocess_file_candidates(
    ...
    match selection {
        ManagedReprocessSelection::All if area == ManagedReprocessArea::ManualLibrary => {
            return Err(Error::InvalidArtifact(
                "full reprocessing is not allowed for Manual Library".into(),
            ));
        }
...
/// Build the exact, model-free Plan that stages root files into Recents.
pub fn build_stage_to_recents_plan(
    source: &Path,
    ...
    build_to_recents_plan(&source, candidates, &folders)
}

/// Build an exact, model-free Plan that returns selected Manual Library or AI
/// Library files to Recents. Classification still happens later through the
/// normal managed Recents workflow.
pub fn build_reprocess_to_recents_plan(
    source: &Path,
    area: ManagedReprocessArea,
    candidates: &[FileCandidate],
) -> Result<Plan, Error> {
...
fn build_to_recents_plan(
    source: &Path,
    candidates: &[FileCandidate],
    folders: &FolderSet,
) -> Result<Plan, Error> {
    let recents_id = folders
        .folders
        .iter()
        .find(|folder| folder.path == RECENTS_DIRECTORY)
        .map(|folder| folder.id.clone())
        .ok_or_else(|| Error::InvalidState("stage FolderSet has no Recents destination".into()))?;
    let classifications = candidates
        .iter()
        .map(|candidate| Classification {
            file_id: candidate.id.clone(),
            destination_id: recents_id.clone(),
            reasoning: None,
            basis: ClassificationBasis::Rule,
            rule_id: Some(STAGE_TO_RECENTS_RULE_ID.into()),
        })
        .collect();
    build_plan(
        source,
...
/// Keep Recents candidates selected by every non-empty eligibility set. This
/// allows callers to filter by ID, path, or the intersection of both.
pub fn filter_recents_candidates(
    candidates: &[FileCandidate],
    ...
    for candidate in candidates {
        validate_recents_candidate(candidate)?;
        if !seen_ids.insert(candidate.id.as_str()) {
            return Err(Error::InvalidArtifact(format!(
                "duplicate Recents candidate ID {:?}",
                candidate.id
            )));
        }
        if !seen_paths.insert(candidate.source_path.as_str()) {
            return Err(Error::InvalidArtifact(format!(
                "duplicate Recents candidate path {:?}",
                candidate.source_path
            )));
        }
    }
    for path in eligible_paths {
        validate_recents_path(path)?;
    }
...
fn stage_folder_set(
    ...
    let folders = Proposal {
        ...
        folders: vec![FolderProposal {
            path: RECENTS_DIRECTORY.into(),
            description: "Files waiting in managed Recents".into(),
        }],
    }
    .approve()?;
    folders.validate()?;
...
fn validate_recents_candidate(candidate: &FileCandidate) -> Result<(), Error> {
    validate_candidate_id(candidate)?;
    validate_recents_path(&candidate.source_path)
}
...
fn validate_recents_path(path: &str) -> Result<(), Error> {
    crate::artifact::normalize_relative_path(path)?;
    let file_name = path
        .strip_prefix(&format!("{RECENTS_DIRECTORY}/"))
        .ok_or_else(|| {
            Error::InvalidArtifact(format!(
                "Recents candidate must start with {RECENTS_DIRECTORY}/: {path:?}"
            ))
        })?;
    if file_name.is_empty() || file_name.contains('/') {
        return Err(Error::InvalidArtifact(format!(
            "Recents candidate must be directly inside Recents: {path:?}"
        )));
    }
    Ok(())
}
...
fn is_managed_area(path: &str) -> bool {
    [
        MANUAL_LIBRARY_DIRECTORY,
        RECENTS_DIRECTORY,
        AI_LIBRARY_DIRECTORY,
    ]
    .iter()
...
mod tests {
    ...
    #[test]
    fn prefixes_ai_library_paths_and_preserves_approved_metadata() {
        let root = tempdir().unwrap();
        let original = approved(root.path());
        let managed = ai_library_folder_set(&original).unwrap();

        assert_eq!(managed.source, original.source);
        assert_eq!(managed.scope.recursive_roots, [RECENTS_DIRECTORY]);
        ...
        for (before, after) in original.folders.iter().zip(&managed.folders) {
            ...
            assert_eq!(
                after.path,
                format!("{AI_LIBRARY_DIRECTORY}/{}", before.path)
            );
        }
    ...
    #[test]
    fn rejects_an_already_ai_library_prefixed_folder_set() {
        let root = tempdir().unwrap();
        let once = ai_library_folder_set(&approved(root.path())).unwrap();
        let error = ai_library_folder_set(&once).unwrap_err();
    ...
    fn scans_only_root_regular_files_with_stable_ids() {
        ...
        fs::write(root.path().join(RECENTS_DIRECTORY), b"reserved").unwrap();
    ...
    #[test]
    fn scans_only_direct_recents_regular_files_and_rejects_a_symlinked_recents() {
        let root = tempdir().unwrap();
        fs::create_dir(root.path().join(RECENTS_DIRECTORY)).unwrap();
        ...
        let candidates = recents_file_candidates(root.path()).unwrap();
        assert_eq!(candidates, [candidate("f000001", "Recents/a.txt")]);

        let other = tempdir().unwrap();
        let linked_root = tempdir().unwrap();
        symlink(other.path(), linked_root.path().join(RECENTS_DIRECTORY)).unwrap();
        assert!(recents_file_candidates(linked_root.path()).is_err());
    }

    #[test]
    fn scans_only_explicit_manual_library_paths_with_stable_deduplication() {
        let root = tempdir().unwrap();
        fs::create_dir(root.path().join(MANUAL_LIBRARY_DIRECTORY)).unwrap();
        fs::create_dir(root.path().join(AI_LIBRARY_DIRECTORY)).unwrap();
        ...
        let candidates =
            reprocess_file_candidates(root.path(), ManagedReprocessArea::ManualLibrary, &selection)
                .unwrap();
    ...
    #[test]
    fn allows_full_ai_library_selection_but_rejects_full_manual_library_selection() {
        let root = tempdir().unwrap();
        fs::create_dir(root.path().join(MANUAL_LIBRARY_DIRECTORY)).unwrap();
        ...
        let error = reprocess_file_candidates(
            root.path(),
            ManagedReprocessArea::ManualLibrary,
            &ManagedReprocessSelection::All,
        )
        .unwrap_err();
    ...
    fn rejects_empty_escaping_and_symlink_reprocessing_selectors() {
        ...
        let linked_root = tempdir().unwrap();
        symlink(
            root.path().join(AI_LIBRARY_DIRECTORY),
            linked_root.path().join(AI_LIBRARY_DIRECTORY),
        )
        .unwrap();
        assert!(
            reprocess_file_candidates(
    ...
    #[test]
    fn builds_a_valid_rule_plan_to_recents_without_model_selection() {
        let root = tempdir().unwrap();
        fs::write(root.path().join("report.txt"), b"report").unwrap();
        let candidates = root_file_candidates(root.path()).unwrap();

        let plan = build_stage_to_recents_plan(root.path(), &candidates).unwrap();
    ...
    #[test]
    fn builds_a_collision_safe_rule_plan_from_ai_library_to_recents() {
        let root = tempdir().unwrap();
        fs::create_dir(root.path().join(MANUAL_LIBRARY_DIRECTORY)).unwrap();
        fs::create_dir(root.path().join(RECENTS_DIRECTORY)).unwrap();
        ...
        .unwrap();
        let plan = build_reprocess_to_recents_plan(
            root.path(),
            ManagedReprocessArea::AiLibrary,
            &candidates,
        )
        ...
        plan.validate().unwrap();
        assert_eq!(plan.scope.recursive_roots, [AI_LIBRARY_DIRECTORY]);
        assert_eq!(
            plan.entries
                .iter()
                .map(|entry| (
    ...
    fn reprocess_plan_rejects_candidates_from_another_area_and_symlinks() {
        ...
        assert!(
            build_reprocess_to_recents_plan(
                root.path(),
                ManagedReprocessArea::AiLibrary,
    ...
    #[test]
    fn applies_and_undoes_an_explicit_manual_library_reprocessing_plan() {
        let root = tempdir().unwrap();
    ...
    fn stage_plan_rejects_nested_reserved_and_symlink_sources() {
        ...
        assert!(
            build_stage_to_recents_plan(root.path(), &[candidate("f1", "Recents/nested.txt")])
                .is_err()
        );
        ...
        .unwrap();
        assert!(
            build_stage_to_recents_plan(root.path(), &[candidate("f1", "linked.txt")]).is_err()
        );
    }
    ...
    fn filters_by_ids_paths_or_their_intersection_and_rejects_escapes() {
        ...
        assert_eq!(
            filter_recents_candidates(&candidates, &ids, &HashSet::new()).unwrap(),
            [candidates[0].clone()]
        );
        assert_eq!(
            filter_recents_candidates(&candidates, &HashSet::new(), &paths).unwrap(),
            candidates
        );
        assert_eq!(
            filter_recents_candidates(&candidates, &ids, &paths).unwrap(),
            [candidates[0].clone()]
        );
        assert!(
            filter_recents_candidates(&candidates, &HashSet::new(), &HashSet::new())
                .unwrap()
                .is_empty()
        );
        assert!(
            filter_recents_candidates(
    ...
    fn empty_stage_candidate_set_builds_an_empty_read_only_plan() {
        ...
        let plan = build_stage_to_recents_plan(root.path(), &[]).unwrap();

        assert!(plan.entries.is_empty());
        assert!(plan.directories.is_empty());
        assert!(!root.path().join(RECENTS_DIRECTORY).exists());
    }
}
