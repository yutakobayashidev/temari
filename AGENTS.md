...
- Treat managed AI Library structure edits as logical immutable `FolderSet` revisions. Preserve existing opaque destination IDs, atomically update monitor/workspace bindings, and leave physical directories and existing files untouched; use explicit reprocessing for file movement.
...
- New workspaces use `Manual Library`, `Recents`, and `AI Library`. Legacy `Kept`, `Inbox`, and `Library` workspaces may be loaded but must change names only through the explicit reviewed migration Plan/Apply/Undo/Resume workflow.
