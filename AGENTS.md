...
- Keep imperative systemd and launchd rendering and installation in `temari-schedule`. A declarative Home Manager module may own systemd user units and use the generation-rooted Nix package directly. Desktop schedule activation must require an explicitly selected stable Temari CLI executable rather than guessing a helper path.
