...
- Treat managed Library structure edits as logical immutable `FolderSet` revisions. Preserve existing opaque destination IDs, atomically update monitor/workspace bindings, and leave physical directories and existing files untouched; use explicit reprocessing for file movement.
...
- Adopt newly created root directories into `Kept` through a read-only setup-style Plan and durable journal; never move them through an unjournaled desktop-only path. Treat a directory manually returned from `Kept`, including by Undo, as durable user intent and derive that identity from authoritative setup and adoption journals rather than a duplicate database record.
