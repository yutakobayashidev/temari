...
- `init` is read-only. It inventories the complete source root and writes a `ManagedSetupPlan` that creates `Manual Library`, `Recents`, and `AI Library`, moves existing real directories to Manual Library, and moves existing regular files to Recents. It rejects reserved-name collisions, special entries, non-portable names, stale source identity, and cross-filesystem directory entries.
- `apply` confirms that exact setup Plan, writes a durable setup journal before mutation, then registers the completed workspace with an AI-Library-prefixed FolderSet. Retention and stability windows are fixed in the workspace definition.
- `run` is finite. It first writes a setup-style Plan for new root directories to `Manual Library`, writes a normal Plan for new root files to Recents, reconciles Recents fingerprints in SQLite, and writes a classification Plan only for files past both deadlines. Directories manually returned from `Manual Library`, including through adoption Undo, are excluded by filesystem identity derived from authoritative setup and adoption journals; missing or invalid referenced journals stop the run before mutation. A different directory that later reuses the same name remains eligible. Without `--apply --yes`, Plans remain reviewable and no file moves. Omitted `--out` creates a private unique artifact directory below the state directory.
- A classified file manually returned from `AI Library` to the workspace root retains its processed filesystem identity and is skipped by later runs. Explicit `reprocess` and recorded Undo remain the only operations that clear that intent as part of their reviewed state transitions.
...
- `reconcile` removes stale pending Recents rows and recognizes manually returned files without treating SQLite as filesystem proof. `status` reports health, physical and indexed Recents counts, eligibility, and actionable runs without contacting the model.
- `reprocess` creates a model-free Plan that stages explicitly selected Manual Library or AI Library files back to Recents. Manual Library always requires area-relative `--path` selectors; AI Library additionally accepts explicit `--all`. Normal retention, rules, model privacy, Plan, Apply, and Undo behavior then applies.
- `schedule print|install|status|uninstall` manages explicit per-user systemd timers or launchd agents through the shared `temari-schedule` adapter. Installed definitions call a stable absolute executable and absolute config/state paths without a shell. `--executable` preserves a user-facing launcher path, while garbage-collectable Nix store paths are rejected. Installation validates before its separate confirmation and rejects environment-backed API keys. Uninstall keeps definitions when the scheduler cannot be stopped. A declarative Home Manager configuration may instead own the systemd user unit and refer directly to the flake or NUR package; its active generation roots the store executable.
- `apply-run` applies a previously recorded directory-adoption, stage, or classification Plan by run ID and exact digest.
...
- Setup Apply indexes the files it moved into Recents immediately. Their retention and stability clocks start from this first local observation, without requiring a priming run or explicit reconciliation.
